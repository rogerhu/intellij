...
public class BlazeApkDeployInfoProtoHelper {
  ...
  /** Extracts relevant information from deploy info proto into data object */
  public static BlazeAndroidDeployInfo extractDeployInfo(
      AndroidDeployInfo deployInfo, File executionRoot, Project project)
      throws GetArtifactsException {
    File mergedManifestFile =
        new File(executionRoot, deployInfo.getMergedManifest().getExecRootPath());
    ParsedManifest mergedManifest = getParsedManifestSafe(mergedManifestFile, project);
    ...
    if (additionalManifests.size() == 1) {
      ...
      testTargetMergedManifest = getParsedManifestSafe(testTargetMergedManifestFile, project);
      ParsedManifestService.getInstance(project)
  ...
  private static ParsedManifest getParsedManifestSafe(File manifestFile, Project project)
      throws GetArtifactsException {
    try {
